...
class SendVistoriaWidget extends StatefulWidget {
  const SendVistoriaWidget({super.key});

  @override
  State<SendVistoriaWidget> createState() => _SendVistoriaWidgetState();
}

class _SendVistoriaWidgetState extends State<SendVistoriaWidget> {
  ...
  _sendDialogTitle() {
    if (vistoriasPageController.sendDialogStatus == SendDialogStatus.enviando) {
      return _sendDialogTitleEnviando();
    }
    if (vistoriasPageController.sendDialogStatus == SendDialogStatus.erro) {
  ...
  _sendAtividade() async {
    ...
    bool allImageSend = vistoriasPageController.allImageSuccessfulSend();
    int sendTries = 0;
    while (sendTries < 3 && !allImageSend) {
      await vistoriasPageController.sendImagesRegistroFoco();
      allImageSend = vistoriasPageController.allImageSuccessfulSend();
      sendTries = sendTries + 1;
    }
    if (!allImageSend) {
      vistoriasPageController.sendDialogStatus = SendDialogStatus.erro;
      if (!dialogDisposed) dialogSetState(() {});
      return;
    }
    bool sendVistoriaSuccessful = false;
    try {
      sendVistoriaSuccessful = await vistoriasPageController.sendVistoria();
    } catch (_) {}
    if (!sendVistoriaSuccessful) {
      vistoriasPageController.sendDialogStatus = SendDialogStatus.erro;
      if (!dialogDisposed) dialogSetState(() {});
      return;
    }
    ...
    vistoriasPageController.sendDialogStatus = SendDialogStatus.concluido;
    dialogSetState(() {});
  }
